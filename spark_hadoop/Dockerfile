...
# modify from https://github.com/datagovsg/python-spark
# TO FIX 
# 1) Setup Java
# 2) Setup sqoop
...
RUN /bin/bash -c 'set -x && \
    ...
    #wget -qO - http://www.apache.org/dist/sqoop/1.4.7/sqoop-1.4.7.bin__hadoop-2.0.4-alpha.tar.gz | tar -xz -C /opt && \
    #cd /opt && ln -s ./sqoop-1.4.7.bin__hadoop-2.0.4-alpha sqoop && \
